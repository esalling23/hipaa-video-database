// Originally writted by Johnny Richardson
// Used here with permission as open source helpers
...
module.exports = function () {
  ...
  _helpers.cloudinaryUrl = function (context, options) {
    ...
    if (options.hash['format'] !== 'svg') options.hash['fetch_format'] = 'auto'
  ...
  _helpers.cdnAsset = function (context, options) {
    ...
    if (options) {
      ...
      let publicId
      let url
      const type = options.hash.fetch ? options.hash.fetch : 'raw'
  ...
  _helpers.img = function (image) {
    if (image.filename == null) return hbs.SafeString('')

    // const path = hbs.escapeExpression(image.path).replace('./public/', '')
    // const filename = hbs.escapeExpression(image.filename)
  ...
  _helpers.fileType = function (file) {
    ...
    if (fileType.indexOf('audio/') !== -1) cssType = 'audio'
    else if (fileType.indexOf('video/') !== -1) cssType = 'video'
    else if (fileType.indexOf('image/') !== -1) cssType = 'image'

    // Find if there is a supported application/ icon
    else {
      const mimeType = file.filetype.replace('application/', '')
    ...
    if (cssType !== undefined) return cssType + '-o'
    else return 'file'
  }
  ...
  _helpers.secureUrl = function (str) {
    if (str !== undefined) return str.replace('http://', 'https://')
    else return str
  }
