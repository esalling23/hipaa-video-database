...
class ResearchLoader {
  ...
  Questions(action) {
    ...
    return new Promise((resolve, reject) => {
      ...
      questionQuery.exec(function(err, questions) {
        if (err || questions.length <= 0) reject("No Questions");
  ...
  MarkerActions(category) {
    ...
    return new Promise((resolve, reject) => {
      ...
      var categoryQuery = Category.model.find({ type: 'Marker Action' })
                                        .populate('parent');
        ...
        if (category) {
          let these = _.filter(categories, function(item) {
      			return item.parent;
      		});
  ...
  MarkerCategories() {
    ...
    return new Promise ((resolve, reject) => {
      ...
      Category.model.find({}, function(err, categories) {
        if (err) reject(err);
        if (categories.length == 0) reject('Error: No Categories');
  ...
  PlacedMarkers(group, categories, actions) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let values = Object.keys(markersByTime).map(function(key) {
          return markersByTime[key];
      });

      // Group markers by category for categorization display
      var groupedMarkers = _.groupBy(_.flatten(values), "category");
  ...
  GetLogs(group, marker) {
    ...
    return new Promise((resolve, reject) => {
    	...
    	const groupQuery = ResearcherResponse.model.find()
    												.populate('question researcher');

      groupQuery.exec(function(err, result) {

        const mostRecent = [];
        const options = _.where(result, { group: group });

        options = _.filter(options, function(opt) {
          return opt.marker == marker;
        });

        const groupedOpt = _.groupBy(options, function(opt) {
